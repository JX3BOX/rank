<template>
    ...
    >
        ...
        <!-- Boss导航 -->
        <el-row
            class="m-rank-boss m-rank-filter m-rank-dps-filter"
            :gutter="20"
            type="flex"
        >
            <el-col
                :span="span"
                v-for="(label, id) of boss_list"
                :key="'aid-' + id"
            >
                <li
                    class="u-boss"
                    ...
                    <div class="u-mount">
                        ...
                        <el-dropdown trigger="click" v-else>
                            <span class="el-dropdown-link">
                                {{ ~~filterMount ? xfmap[filterMount] : '全部心法' }}<i
            ...
            <template v-for="(item, i) in list">
                ...
                >
                    ...
                    <el-col :span="2"
                        ><div
                            class="u-mount"
                            :style="{ color: showMountColor(item.mount) }"
                        >
                            <img :src="item.mount | showMountIcon" />{{
                                item.mount | showMountLabel
                            }}
                        </div></el-col
                    >
                    <el-col :span="3"
                    ...
                    <el-col :span="3"
                        ><a
                            class="u-team"
                            :href="item.team_id | showTeamLink"
                            target="_blank"
                            v-if="item.team_id"
                            ><img :src="item | showTeamLogo" /><span>{{
                                item | showTeamName
                            }}</span></a
                    ...
                    <el-col :span="4"
                        ><div class="u-role">
                            <a
                                :href="item.uid | authorLink"
                                target="_blank"
                                v-if="item.uid"
                                ><img :src="item | showUserAvatar" />{{
                                    item.role
                                }}</a
                            >
                    ...
                    <el-col
                        :span="4"
                        :class="i > 10 ? `u-dps-10` : `u-dps-${i}`"
                        ><div
                            class="u-dps u-bar"
                            :style="{ background: showMountColor(item.mount) }"
                        >
                            {{ item._dhps }}
                        </div></el-col
                    >
...
import server_std from "@jx3box/jx3box-data/data/server/server_std";
